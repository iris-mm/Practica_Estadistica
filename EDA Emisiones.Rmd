...
Se crearán tablas de contingencia para explorar la relación entre dos variables cualitativas, como Engine type y Market segment, lo que permitirá ver si ciertos tipos de motor están asociados a tipos específicos de vehículos.

--------Gráfico de la distribución del Índice de Eficiencia del Combustible--------
```{r}
tabla_contingencia <- table(emisiones$`Engine type`, emisiones$`Market segment`)
print(tabla_contingencia)
```

### 3.2.2. Gráfico de dispersión

Se usarán gráficos de dispersión para explorar relaciones entre variables cuantitativas, como el consumo de combustible (Max Consumption l/100km) y las emisiones de CO2 (Max emissions gCO2/km), o entre el área frontal (Área frontal) y las emisiones de CO2.
...
ggplot(as.data.frame(tabla_frecuencias_fuel_economy), aes(x = Var1, y = Freq)) +
  geom_bar(stat = "identity", fill = COLOR_EconomyIndex, alpha = 0.8) +
