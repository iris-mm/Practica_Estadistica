...
Antes de comenzar el análisis, se han descartado 8 de las 26 variables que ofrece el dataset, ya que no se consideran relevantes para el estudio o contienen una muestra de datos demasiado pequeña. Las variables excluidas son las siguientes:
...
ggplot(emisiones, aes(x = `engine_type`)) +
  geom_bar(fill = COLOR_EngineType) +
...
ggplot(emisiones, aes(x = `gross_vehicle_weight_rating_kg`)) +
  geom_histogram(binwidth = 100, fill = COLOR_Weigth, color = "black", alpha = 0.7) +
...
ggplot(emisiones, aes(x = `fuel_economy_index`)) +
  geom_bar(fill = COLOR_EconomyIndex) +
...
En esta parte se analizarán las relaciones entre pares de variables cuantitativas mediante el cálculo de coeficientes de correlación y covarianza. Estos indicadores permitirán entender la fuerza y dirección de la relación entre dos variables.

 *Correlación de Pearson*
La correlación de Pearson mide la intensidad y la dirección de una relación lineal entre dos variables cuantitativas. Los valores de la correlación oscilan entre -1 y 1, donde:
...
En este estudio, se calculará la correlación de Pearson para estudiar las siguientes relaciones:
...
- La correlación de Pearson calculada entre el consumo máximo de combustible y las emisiones máximas de CO2 ha dado un valor de 0.8216394, lo que indica una relación lineal positiva fuerte entre estas dos variables.Esto implica que a medida que el consumo máximo de combustible de un vehículo aumenta, también lo hacen las emisiones máximas de CO2.Este resultado es coherente con lo que cabría esperar en un estudio de emisiones de CO2 de vehículos, ya que los automóviles que tienen un mayor consumo de combustible tienden a ser menos eficientes energéticamente y, por tanto, emiten más gases contaminantes como el CO2.
- La correlación de Pearson calculada entre la potencia en CV y las emisiones máximas de CO2 ha dado un valor de 0.3801142, lo que indica una relación lineal positiva moderada entre estas dos variables.Esto significa que, aunque existe una tendencia a que los vehículos con mayor potencia en cv tienden a emitir más CO2, la relación no es muy fuerte, y hay otros factores que también influyen significativamente en las emisiones de CO2, además de la potencia del vehículo. Aunque hay una correlación positiva entre la potencia del motor en CV y las emisiones de CO2 esta relación es más débil en comparación con la relación entre consumo de combustible y emisiones. Esto sugiere que, aunque la potencia es un factor a tener en cuenta, no es el principal determinante de las emisiones de CO2.

*Covarianza*
La covarianza indica cómo varían juntas dos variables. A diferencia de la correlación, la covarianza no está acotada entre -1 y 1, lo que significa que su valor depende de las unidades de las variables.
...
En el caso de las variables del estudio:

Se calculará la covarianza entre Max Consumption l/100km y Max emissions gCO2/km para ver cómo varían juntas estas dos variables y si un mayor consumo implica una mayor emisión de CO2.
...
Para el cálculo de las tablas emplearemos los valores medios de cada variable cuantitativa, ya que si usasemos todos los del dataset quedarían tablas demasiado grandes y estas perderían su función de resumir la información.
...
Con esta tabla podemos concluir que la mayoría de vehículos grandes (Camión, Furgón, Chasis-Cabina Grande) tienden a contaminar más que los pequeños ya que poseen valores mayores para el consumo de combustible y las emisiones. Esto confirma nuestros estudios previos relacionados con el área frontal y las emisiones máximas de los vehículos.
...
En esta tabla visualizamos fácilmente como los vehículos de transmisión semiautomática (SC) consumen la mayor cantidad de combustible de todas las categorías y sin embargo, son los que tienen transmisión automática (A) o manual (M) los que emiten más CO2. Esto sugiere que los coches más modernos como pueden ser los automáticos han conseguido un mejor ahorro del combustible pero no rebajar la cantidad de emisiones al medio; mientras que los semiautomáticos son menos sostenibles puesto que consumen grandes cantidades de combustible pese a emitir menos CO2.
...
Con la tabla se nos confirma que existe una relación directamente proporcional entre el tamaño de la cilindrada y el consumo y emisiones del vehículo, de forma que a mayor cilindrada, mayores son los otros dos valores.\
